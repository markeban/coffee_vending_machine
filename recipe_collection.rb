...
class RecipeCollection
  ...
  class Recipe
    Ingredient = Struct.new(:base_ingredient_id, :unit_count, keyword_init: true)
    ...
    def initialize(recipe_data)
      ...
      @ingredients = recipe_data.fetch(:ingredients).map do |ingredient_data|
        Ingredient.new(
          base_ingredient_id: ingredient_data.fetch(:base_ingredient_id), 
          unit_count: ingredient_data.fetch(:unit_count)
        )
      end
    end
  end
